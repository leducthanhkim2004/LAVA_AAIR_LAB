...
def get_OT_dual_sol(feature_extractor, trainloader, testloader, training_size=5000, p=2, resize=32, device='cuda'):
    print(f"[DEBUG OT] training_size = {training_size}")
    print(f"[DEBUG OT] trainloader batch size = {trainloader.batch_size}, num batches = {len(trainloader)}")
    print(f"[DEBUG OT] testloader batch size = {testloader.batch_size}, num batches = {len(testloader)}")
    try:
        print(f"[DEBUG OT] train dataset length = {len(trainloader.dataset)}")
        print(f"[DEBUG OT] test dataset length = {len(testloader.dataset)}")
    except:
        pass
